...
import { applyPolyfills, defineCustomElements } from '../../elements/dist/loader';
import {needsKebabCase, setupConfig} from './utils';
...
*/
const toLowerCase = (eventName: string) => eventName === 'inoChange' ? 'v-inochange' : eventName.toLowerCase();
const toKebabCase = (eventName: string) => eventName === 'inoChange' ? 'v-ino-change' : eventName.replace(/([a-z0-9]|(?=[A-Z]))([A-Z])/g, '$1-$2').toLowerCase();
